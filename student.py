...
class Piggy(pigo.Pigo):
    ...
    def __init__(self):
        ...
        self.SAFE_STOP_DIST = 45 #if it is too small then it will get confused
    ...
    def direction_change(self):
        """this method helps the robot change direction, it takes the left and right totals to decide where to turn"""
        if self.is_clear():
            self.cruise()
    ...
    def cruise(self):
        ...
        while self.dist() > self.SAFE_STOP_DIST:
            time.sleep(.001) #making it only sleep for a minimal amount of time
        self.stop()
